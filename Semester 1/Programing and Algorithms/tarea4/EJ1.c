...
void gradient_image(unsigned char **image, int rows, int cols, char *newFn){
	//Partimos creando la ventana
	int wsz=3;
	unsigned char **win=crea_img(wsz, wsz);
	//Creamos una matriz para ir guardando los resultados
...
unsigned char mat_mean(unsigned char **matriz, int rows, int cols){
	double sum=0.0;
...
double mat_grad(unsigned char **win){
	//Se define el operador Sobel
	unsigned char **sobelX = crea_img(3,3);
	unsigned char **sobelY = crea_img(3,3);
	sobelX[0][0] = sobelY[0][0] = -1;
	sobelX[1][0] = sobelY[0][1] = -2;
	sobelX[2][0] = sobelY[0][2] = -1;
	sobelX[0][2] = sobelY[2][0] = 1;
	sobelX[1][2] = sobelY[2][1] = 2;
	sobelX[2][2] = sobelY[2][2] = 1;
	...
	for(int i=0; i<3; i++){
		for(int j=0; j<3; j++){
			x+=sobelX[i][j]*win[i][j];
			y+=sobelY[i][j]*win[i][j];
		}
	}
